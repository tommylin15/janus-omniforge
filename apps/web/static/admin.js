...
const state = { page: 0, limit: 10, hasNext: false, stocks: [], selected: new Set() };
...
async function loadStocks() {
  const params = new URLSearchParams({ q: byId("stock-query").value.trim(), limit: state.limit, offset: state.page * state.limit });
...
function renderStocks() {
  ...
  byId("page-label").textContent = `第 ${state.page + 1} 頁`;
  byId("prev-page").disabled = state.page === 0;
  byId("next-page").disabled = !state.hasNext;
  selectionChanged();
}
...
function openStock(stock = null) {
  ...
  const dialog = byId("stock-dialog"); dialog.dataset.editing = stock?.symbol || ""; dialog.showModal(); byId("stock-name").focus();
}
...
async function openStatus(stock) {
  try { const data = await request(`/api/v1/admin/stocks/${encodeURIComponent(stock.symbol)}/status`); const content = byId("status-content"); content.replaceChildren(); Object.entries(data.summary || {}).forEach(([key, value]) => { const row = document.createElement("p"); row.append(node("strong", `${key}: `), node("span", typeof value === "object" ? JSON.stringify(value) : value ?? "—")); content.append(row); }); byId("status-dialog").showModal(); }
  catch (error) { showNotice(error.message, true); }
}
...
async function loadExecutions() {
  ...
  try {
    const data = await request("/api/v1/admin/executions?limit=50");
    body.replaceChildren();
    ...
    data.items.forEach((execution) => {
      ...
      detail.addEventListener("click", () => openExecution(execution.execution_id, detail));
...
async function openExecution(id, opener) {
  try {
    ...
    const dialog = byId("execution-dialog"); dialog.dataset.opener = opener.id || ""; dialog.showModal(); byId("close-dialog").focus();
  } catch (error) { showNotice(error.message, true); }
}
...
async function loadCatalog() {
  ...
  try { const data = await request("/api/v1/admin/source-catalog"); list.replaceChildren(); if (!data.items.length) list.append(node("p", "尚無資料源設定", "empty")); data.items.forEach((config) => { const card = node("article", null, "source-card"); card.append(node("strong", `${config.config_id} · ${config.dataset_id}`), node("p", `來源 ${config.source_ids.join(", ")} · ${config.cadence} · ${config.coverage_tier}`), statusBadge(config.authorization_status)); list.append(card); }); }
...
async function refreshAll() { await Promise.all([loadStocks(), loadExecutions(), loadSources(), loadCatalog(), loadMembership(), loadSettings()]); }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  byId("stock-search").addEventListener("submit", (event) => { event.preventDefault(); state.page = 0; loadStocks(); });
  byId("enabled-filter").addEventListener("change", () => { state.page = 0; loadStocks(); });
  byId("prev-page").addEventListener("click", () => { if (state.page > 0) { state.page -= 1; loadStocks(); } });
  byId("next-page").addEventListener("click", () => { if (state.hasNext) { state.page += 1; loadStocks(); } });
  ...
  byId("close-status").addEventListener("click", () => byId("status-dialog").close());
  byId("execution-dialog").addEventListener("click", (event) => { if (event.target === byId("execution-dialog")) byId("execution-dialog").close(); });
