...
class Api {
  ...
  Future<List<Map<String,dynamic>>> events(String thread,int cursor) async {
    final request=http.Request('GET',Uri.parse('$base/api/v1/me/chats/threads/$thread/events?cursor=$cursor&limit=200'))..headers.addAll(headers);
    final response=await request.send(); if(response.statusCode<200||response.statusCode>=300)throw Exception('事件串流暫時無法使用');
    final result=<Map<String,dynamic>>[]; String? event; String? id; final data=StringBuffer();
    void flush(){if(data.length==0)return; final value=jsonDecode(data.toString()) as Map<String,dynamic>; result.add({...value,'event_type':event,'seq':int.tryParse(id??'')??cursor}); event=null;id=null;data.clear();}
    await for(final line in response.stream.transform(utf8.decoder).transform(const LineSplitter())){if(line.isEmpty){flush();}else if(line.startsWith('event: ')){event=line.substring(7);}else if(line.startsWith('id: ')){id=line.substring(4);}else if(line.startsWith('data: ')){data.write(line.substring(6));}}
    flush(); return result;
  }
...
class _WorkspaceState extends State<Workspace> {
  ...
  @override Widget build(BuildContext context) {
    final pages=[WatchlistPage(widget.api),JournalNotesPage(widget.api),ChatPage(widget.api),ProfilePage(api:widget.api,email:widget.email,onTheme:widget.onTheme)];
    return Scaffold(appBar:AppBar(title:const Text('Janus')),body:pages[page],bottomNavigationBar:NavigationBar(selectedIndex:page,
      ...
      NavigationDestination(icon:Icon(Icons.edit_note),label:'記帳／筆記'),NavigationDestination(icon:Icon(Icons.chat_bubble_outline),label:'AI'),NavigationDestination(icon:Icon(Icons.person_outline),label:'我的')]));
  }
}

class ChatPage extends StatefulWidget { const ChatPage(this.api,{super.key}); final Api api; @override State<ChatPage> createState()=>_ChatPageState(); }
class _ChatPageState extends State<ChatPage> {
  late Future<dynamic> threads=widget.api.get('/api/v1/me/chats/threads');
  Map<String,dynamic>? thread; final input=TextEditingController(); final events=<String,Map<String,dynamic>>{}; int cursor=-1; bool busy=false; String runtime='gemini'; String model='gemini-2.5-flash';
  @override void dispose(){input.dispose();super.dispose();}
  void reload()=>setState(()=>threads=widget.api.get('/api/v1/me/chats/threads'));
  Future<void> createThread() async { final value=await widget.api.post('/api/v1/me/chats/threads',{'runtime':runtime,'model':model,'assistant_profile':'default'}); setState((){thread=value;events.clear();cursor=-1;}); reload(); }
  Future<void> select(Map<String,dynamic> value) async {setState((){thread=value;events.clear();cursor=-1;busy=true;});try{for(final event in await widget.api.events(value['thread_id'], -1)){_merge(event);}}finally{if(mounted)setState(()=>busy=false);}}
  void _merge(Map<String,dynamic> event){final seq=event['seq'] as int? ?? -1;if(seq<=cursor)return;cursor=seq;final id=event['event_id']?.toString()??'$seq';final item=event['item_id']?.toString()??id;final type=event['event_type']?.toString();if(type=='item_upsert')events[item]={...events[item]??{},...event};else events[id]=event;}
  Future<void> send() async {final value=input.text.trim();if(value.isEmpty||thread==null||busy)return;input.clear();setState(()=>busy=true);try{final response=await widget.api.post('/api/v1/me/chats/threads/${thread!['thread_id']}/messages',{'content':value});final turn=response['turn'] as Map<String,dynamic>?;for(var attempt=0;attempt<10;attempt++){for(final event in await widget.api.events(thread!['thread_id'],cursor)){_merge(event);}if(turn?['status']!='RUNNING'||events.values.any((e)=>e['event_type']=='turn_completed'||e['event_type']=='turn_error'||e['event_type']=='turn_cancelled'))break;await Future<void>.delayed(const Duration(milliseconds:300));}}catch(_){if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('訊息送出失敗，請稍後重試')));}finally{if(mounted)setState(()=>busy=false);}}
  Future<void> approval(Map<String,dynamic> event,bool approved) async {final payload=Map<String,dynamic>.from(event['payload'] as Map? ?? {});final id=payload['request_id']?.toString()??payload['requestId']?.toString();final turn=event['turn_id']?.toString();final digest=payload['params_digest']?.toString()??payload['paramsDigest']?.toString();if(id==null||turn==null||digest==null)return;await widget.api.post('/api/v1/me/chats/threads/${thread!['thread_id']}/turns/$turn/approvals/$id',{'approved':approved,'params_digest':digest});}
  @override Widget build(BuildContext context)=>LayoutBuilder(builder:(context,box)=>Row(children:[SizedBox(width:box.maxWidth<700?150:230,child:_ThreadList()),Expanded(child:thread==null?_StartChat(onCreate:createThread):_Room(onSend:send,onApproval:approval,input:input,busy:busy,events:events.values.toList(),runtime:runtime,model:model,onRuntime:(v){setState((){runtime=v;model=v=='gemini'?'gemini-2.5-flash':v=='openrouter'?'openai/gpt-4o-mini':'gpt-5';});},onTools:()=>showModalBottomSheet(context:context,builder:(_)=>AssistantControls(widget.api))))]));
  Widget _ThreadList()=>Column(children:[Padding(padding:const EdgeInsets.all(8),child:FilledButton.icon(onPressed:createThread,icon:const Icon(Icons.add),label:const Text('新對話'))),Expanded(child:FutureBuilder(future:threads,builder:(context,snapshot){if(!snapshot.hasData)return const Center(child:CircularProgressIndicator());final data=snapshot.data;final values=(data is Map ? data['items'] as List? : null)??[];return ListView(children:[for(final value in values)ListTile(selected:value['thread_id']==thread?['thread_id'],title:Text('${value['runtime']} · ${value['model']}'),subtitle:Text(value['thread_id']),onTap:()=>select(Map<String,dynamic>.from(value)))]);}))]);
}

class _StartChat extends StatelessWidget { const _StartChat({required this.onCreate}); final VoidCallback onCreate; @override Widget build(BuildContext context)=>Center(child:FilledButton.icon(onPressed:onCreate,icon:const Icon(Icons.chat),label:const Text('建立私人助理對話'))); }
class _Room extends StatelessWidget { const _Room({required this.onSend,required this.onApproval,required this.input,required this.busy,required this.events,required this.runtime,required this.model,required this.onRuntime,required this.onTools}); final VoidCallback onSend; final Future<void> Function(Map<String,dynamic>,bool) onApproval; final TextEditingController input; final bool busy; final List<Map<String,dynamic>> events; final String runtime,model; final ValueChanged<String> onRuntime; final VoidCallback onTools;
  @override Widget build(BuildContext context)=>Column(children:[Padding(padding:const EdgeInsets.fromLTRB(12,8,12,0),child:Row(children:[Expanded(child:DropdownButtonFormField<String>(initialValue:runtime,decoration:const InputDecoration(labelText:'Runtime'),items:const [DropdownMenuItem(value:'gemini',child:Text('Gemini API')),DropdownMenuItem(value:'openrouter',child:Text('OpenRouter')),DropdownMenuItem(value:'codex',child:Text('Codex'))],onChanged:(v){if(v!=null)onRuntime(v);})),const SizedBox(width:8),IconButton(tooltip:'MCP／Skills／資料源',onPressed:onTools,icon:const Icon(Icons.tune))])),Expanded(child:events.isEmpty?const Center(child:Text('開始輸入訊息')):ListView(padding:const EdgeInsets.all(12),children:[for(final event in events)_EventCard(event,onApproval)])),SafeArea(top:false,child:Padding(padding:const EdgeInsets.all(12),child:Row(crossAxisAlignment:CrossAxisAlignment.end,children:[Expanded(child:TextField(controller:input,minLines:1,maxLines:5,decoration:const InputDecoration(hintText:'詢問私人助理…',border:OutlineInputBorder()))),const SizedBox(width:8),IconButton(onPressed:busy?null:onSend,icon:busy?const SizedBox.square(dimension:22,child:CircularProgressIndicator(strokeWidth:2)):const Icon(Icons.send))])))]);
}
class _EventCard extends StatelessWidget { const _EventCard(this.event,this.onApproval); final Map<String,dynamic> event; final Future<void> Function(Map<String,dynamic>,bool) onApproval; @override Widget build(BuildContext context){final payload=Map<String,dynamic>.from(event['payload'] as Map? ?? {});final type=event['event_type']?.toString()??'';final content=payload['content']?.toString()??payload['text']?.toString()??payload['message']?.toString()??payload.toString();final role=payload['role']?.toString();if(type=='approval_request')return Card(color:Theme.of(context).colorScheme.secondaryContainer,child:ListTile(leading:const Icon(Icons.lock_outline),title:const Text('需要核准的操作'),subtitle:Text(content),trailing:Wrap(children:[TextButton(onPressed:()=>onApproval(event,false),child:const Text('拒絕')),FilledButton(onPressed:()=>onApproval(event,true),child:const Text('允許'))]));if(type=='turn_completed'||type=='turn_cancelled'||type=='turn_error')return Padding(padding:const EdgeInsets.all(8),child:Text(type=='turn_completed'?'已完成':type=='turn_cancelled'?'已取消':'執行失敗'));return Align(alignment:role=='user'?Alignment.centerRight:Alignment.centerLeft,child:Card(child:Padding(padding:const EdgeInsets.all(12),child:MarkdownText(content))));}}
class MarkdownText extends StatelessWidget { const MarkdownText(this.value,{super.key}); final String value; @override Widget build(BuildContext context){var code=false;final children=<Widget>[];for(final line in value.split('\n')){if(line.startsWith('```')){code=!code;continue;}children.add(Padding(padding:const EdgeInsets.only(bottom:4),child:line.startsWith('# ')?Text(line.substring(2),style:Theme.of(context).textTheme.titleLarge):line.startsWith('**')&&line.endsWith('**')?Text(line.substring(2,line.length-2),style:const TextStyle(fontWeight:FontWeight.bold)):code?Container(width:double.infinity,color:Theme.of(context).colorScheme.surfaceContainerHighest,padding:const EdgeInsets.all(8),child:Text(line,style:const TextStyle(fontFamily:'monospace',color:Colors.cyan))):Text(line)));}return Column(crossAxisAlignment:CrossAxisAlignment.start,children:children);}}
class AssistantControls extends StatefulWidget { const AssistantControls(this.api,{super.key}); final Api api; @override State<AssistantControls> createState()=>_AssistantControlsState(); }
class _AssistantControlsState extends State<AssistantControls> { late Future<dynamic> mcp=widget.api.get('/api/v1/me/mcp/servers'), skills=widget.api.get('/api/v1/me/skills');
  Future<void> toggleSkill(Map<String,dynamic> value,bool enabled) async {await widget.api.put('/api/v1/me/skills/${value['skill_id']}/state',{'enabled':enabled,'revision':value['revision']});setState(()=>skills=widget.api.get('/api/v1/me/skills'));}
  @override Widget build(BuildContext context)=>SafeArea(child:ListView(padding:const EdgeInsets.all(16),shrinkWrap:true,children:[Text('資料源、MCP 與 Skills',style:Theme.of(context).textTheme.titleLarge),const SizedBox(height:8),_Panel('MCP Servers',mcp,(v)=>'${v['server_id']} · ${v['enabled']==true?'已啟用':'未啟用'}'),_TogglePanel('Skills',skills,(v)=>'${v['skill_id']} · revision ${v['revision']}',toggleSkill),const ListTile(leading:Icon(Icons.info_outline),title:Text('私人資料須逐項選取'),subtitle:Text('API key、Codex auth 與資料庫憑證不會進入裝置。'))])); }
class _Panel extends StatelessWidget { const _Panel(this.title,this.future,this.label); final String title; final Future<dynamic> future; final String Function(Map<String,dynamic>) label; @override Widget build(BuildContext context)=>FutureBuilder(future:future,builder:(context,snapshot){final data=snapshot.data;final values=(data is Map ? data['items'] as List? : null)??[];return ExpansionTile(title:Text(title),children:values.isEmpty?[const ListTile(title:Text('目前沒有設定'))]:[for(final value in values)ListTile(title:Text(label(Map<String,dynamic>.from(value))))]);}); }
class _TogglePanel extends StatelessWidget { const _TogglePanel(this.title,this.future,this.label,this.onToggle); final String title; final Future<dynamic> future; final String Function(Map<String,dynamic>) label; final Future<void> Function(Map<String,dynamic>,bool) onToggle; @override Widget build(BuildContext context)=>FutureBuilder(future:future,builder:(context,snapshot){final data=snapshot.data;final values=(data is Map ? data['items'] as List? : null)??[];final children=values.map((raw){final value=Map<String,dynamic>.from(raw);return SwitchListTile(title:Text(label(value)),value:value['enabled']==true,onChanged:(enabled)=>onToggle(value,enabled));}).toList();return ExpansionTile(title:Text(title),children:children.isEmpty?[const ListTile(title:Text('目前沒有設定'))]:children);}); }
