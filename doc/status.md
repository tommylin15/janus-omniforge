...
- **`WBS-8-DEV-PILOT-RUN` 已進入 operational evidence window，但目前為 partial／blocked checkpoint。**Pilot 起點後的 Scheduler execution `janus-ingestion-core-n8bs2` 於 2026-09-25T23:30:01Z 啟動，最終 `Completed=False`／`failedCount=1`；兩次 task attempt 都在 `twse-valuation`、`twse-institutional`、`twse-market-activity` 對 `2026-09-25` 產生 `ValueError`。該日為 TWSE 官方休市日，runtime 卻未由既有 `schedule.holiday_overrides` 回推到前一交易日。版本化修復 `029_twse_2026_holiday_overrides.sql` 已準備完成，但尚未套用：Cloud Build identity 缺 IAP，GitHub `janus-ci` 缺 `compute.instances.get`；沒有擴大 IAM。下一入口是 [`runbook-pilot-calendar-repair.md`](runbook-pilot-calendar-repair.md) 的既有 operator IAP 路徑，套用後仍需 bounded live ingestion acceptance。
...
1. **繼續 `WBS-8-DEV-PILOT-RUN` 六個月 evidence window。**
   - 先依 `runbook-pilot-calendar-repair.md` 用既有 operator IAP identity 套用 029，確認 `2026-09-25`／`2026-09-28` 進入 `holiday_overrides`，再執行受影響 TWSE dataset 的 bounded live ingestion acceptance；未完成前維持 partial／blocked。
