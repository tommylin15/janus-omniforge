...
- **`WBS-8-DEV-PILOT-RUN` 已進入 operational evidence window，目前仍為 partial。**Pilot 起點後的 Scheduler execution `janus-ingestion-core-n8bs2` 曾在 TWSE 休市日 `2026-09-25` 對 `twse-valuation`、`twse-institutional`、`twse-market-activity` 產生 `ValueError`。版本化修復 `029_twse_2026_holiday_overrides.sql` 已由既有 operator IAP 路徑成功套用，migration marker 與 `2026-09-25`／`2026-09-28` holiday overrides 已有 live evidence。三個受影響 dataset 的 bounded live acceptance 現已全部通過：`twse-valuation` execution `janus-ingestion-core-7z8kv` 成功回推到 `2026-09-24`；`twse-institutional` GitHub Actions run `36216088848`／execution `janus-ingestion-core-pwgn9` 最終 `Completed=True`／`succeededCount=1`，並保留第一次 transient `HTTPError` 後 retry 成功的真實 evidence；`twse-market-activity` GitHub Actions run `36218341537`／execution `janus-ingestion-core-zvbc9` 亦以 `Completed=True`／`succeededCount=1`／attempt 0 container `exit(0)` 完成，requested `2026-09-26` 正確解析為 `as_of=2026-09-24`、`dates=[2026-09-24]`，application summary 為 `failed=0`、`status=succeeded`、`staged=1`、`core_created=12`，`core.market_activity_v1` snapshot 為 336 rows，Mart trigger `not_required`。本輪未重現原 calendar `ValueError`，因此 calendar repair 的三個 dataset bounded acceptance 切片完成；但 `WBS-8-DEV-PILOT-RUN` 仍須累積六個 calendar months 的 operational evidence，不能宣稱整體完成。
...
1. **繼續 `WBS-8-DEV-PILOT-RUN` 六個月 evidence window。**
   - 交易日曆修復的 `twse-valuation`、`twse-institutional`、`twse-market-activity` 三個 bounded live acceptance 已完成。
   - 起點 `pilot_started_at=2026-09-24T15:29:19Z`；後續仍需記錄真實 Scheduler／ingestion／analysis、outcome、usefulness、cost、操作介入、失敗與 security／privacy evidence。
...
3. 本頁做快速定位。
4. [`spec/operations-and-testing.md`](spec/operations-and-testing.md) 查完整 evidence ledger 與歷史 checkpoint；交易日曆修復的可重跑 procedure 與本輪 bounded acceptance evidence 同時記於 [`runbook-pilot-calendar-repair.md`](runbook-pilot-calendar-repair.md)。
