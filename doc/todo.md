...
- omniAgent split UI ownership：generic Flutter Chat source／widget tests 已移至 omniAgent；Janus Flutter source 僅保留投資與 Admin 頁，移除 AI 導航與個股 Ask Janus 入口。這是 source checkpoint，既有 dev deployment／Janus Chat API 仍維持現況；omniAgent OAuth、runtime dispatch、tools／Skills 管理、Janus context 與 live cutover 尚未驗收。
