...
- Phase 5 UI source split 已完成：generic Flutter Chat source／widget tests 由 omniAgent 持有；Janus Flutter source 僅保留投資與 Admin 頁。deployment safety 的 source 修正已固定拆分前 User App Web artifact，讓後續 Janus API image 保留舊 Chat UI；本地 Flutter 驗證與最小 CI workflow 已補，GCP dev 新 image／CI 尚待實際驗收。現有舊 `usefulness-rollback` image 已被 registry 清理；最近兩版保留政策也無法支撐先做零流量候選、再自動部署後仍保有目前 live image。舊 API rollback artifact 保留方案未落地前不部署。Janus 仍是 live Chat writer；omniAgent OAuth、runtime dispatch、Janus context、Skills/MCP、historical migration、live cutover 未完成，Phase 5 不標記 completed。
