...
catalog, publication, audit, Web control, Web catalog, and Web publication Secret Manager values to
...
It accepts only an immutable PostgreSQL digest, reads the three Web passwords from
stdin, replaces the running container while preserving the data volume, applies
migrations `007`, `020`, and `021`, and restores the previous image automatically if the rollout
fails.
