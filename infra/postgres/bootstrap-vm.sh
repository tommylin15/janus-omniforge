...
sudo docker exec \
  ...
  janus-postgres bash -ceu '
    ...
    rm -f /tmp/vars.sql /tmp/web-vars.sql
...
sudo rm -f "$env_file"
unset bootstrap_password control_password catalog_password publication_password audit_password web_control_password web_catalog_password
